        ...
        if (currentEssence < this.getCostPerRefresh() * list.size()) {
            ...
            for (EntityLivingBase livingEntity : list) {
                ...
                if (potionEffects.isEmpty()) {
                    ...
                    if (hasOffensa) {
                        if (!potionEffects.contains(new PotionEffect(Potion.weakness.id, 5, 1)))
                            livingEntity.addPotionEffect(new PotionEffect(Potion.weakness.id, 5, 1));
                    }

                    if (!potionEffects.contains(new PotionEffect(Potion.moveSlowdown.id, 5, 9)))
                        livingEntity.addPotionEffect(new PotionEffect(Potion.moveSlowdown.id, 5, 9));
                    if (!potionEffects.contains(new PotionEffect(Potion.digSlowdown.id, 5, 4)))
                        livingEntity.addPotionEffect(new PotionEffect(Potion.digSlowdown.id, 5, 4));
                }

                if (livingEntity instanceof EntityEnderman) {
                    if (!potionEffects.contains(new PotionEffect(AlchemicalWizardry.customPotionPlanarBindingID, 5, 0)))
                        livingEntity.addPotionEffect(
                                new PotionEffect(AlchemicalWizardry.customPotionPlanarBindingID, 5, 0));
