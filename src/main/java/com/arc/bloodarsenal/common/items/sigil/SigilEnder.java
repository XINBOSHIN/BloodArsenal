...
public class SigilEnder extends EnergyItems implements IBindable, ISigil {
    ...
    public ItemStack onItemRightClick(ItemStack itemStack, World world, EntityPlayer player) {
        ...
        if (player.isSneaking() && !player.isSwingInProgress) {
            MovingObjectPosition mop = SigilUtils.getTargetBlock(
                    world,
            ...
            if (mop != null && mop.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK && mop.sideHit != -1) {
                ...
                if (SoulNetworkHandler.getCurrentEssence(itemStack.getTagCompound().getString("ownerName"))
                    ...
                    for (int k = 0; k < 8; ++k) {
                        ...
                        world.spawnParticle(
                                "portal",
                                ex + (world.rand.nextDouble() - 0.5D) * (double) player.width,
                                wy + world.rand.nextDouble() * (double) player.height - 0.25D,
                                zee + (world.rand.nextDouble() - 0.5D) * (double) player.width,
                                0,
                                0,
                                0);
                    }
