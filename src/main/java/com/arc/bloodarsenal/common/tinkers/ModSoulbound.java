...
public class ModSoulbound extends ModBoolean {
    ...
    public boolean matches(ItemStack[] input, ItemStack tool) {
        ...
        for (ItemStack stack : input) {
            ...
            if (!(stack.getItem() instanceof IBloodOrb)) continue;
            if (!(((IBloodOrb) stack.getItem()).getOrbLevel() >= 4)) continue;
            if (SoulNetworkHandler.getOwnerName(stack).equals("")) continue;
            if (foundOrb != null) return false;
